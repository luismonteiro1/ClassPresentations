...
::: notes
- Offset fáz:
   - com que o output do neuron seja 1, se a soma dos dos inputs for maior que o offset, se não o output é 0
   - Ou seja o offset representa o valor minimo que a soma tem de ter para o neuron activar
:::
...
::: notes
em duas dimensões a divisão é feita por uma linha, em tres dimensoes por um plano
   - Em dimensoes maiores a divisão é feita por um hyperplane
   - Isto significa que um unico neuronio só consegue representar funções que sejam linearmente separaveis 
(isto é uma limitação muito grande no numero de funções que podemos representar)
:::
